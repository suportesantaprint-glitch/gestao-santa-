...
function addDateRange(
  ...
  addParam(params, field, start ? `gte.${start}` : undefined);
  // Use the next midnight as an exclusive upper bound. This includes every
  // timestamp on the selected final day, including fractional seconds.
  addParam(params, field, end ? `lt.${nextDate(end)}` : undefined);
}
...
function chamadasFilters(query: Record<string, string>): URLSearchParams {
  ...
  addDateRange(params, "emissao", query.dataInicio, query.dataFim);
  addParam(
    params,
...
function pecasFilters(query: Record<string, string>): URLSearchParams {
  ...
  addDateRange(params, "data_abertura", query.dataInicio, query.dataFim);
  return params;
}
...
async function dashboardResumo(query: Record<string, string>): Promise<unknown> {
  ...
  if (unavailable.length === results.length) {
    ...
    throw firstFailure?.reason ?? new Error("Não foi possível consultar os indicadores.");
  }

  const values = results.map((result) => (result.status === "fulfilled" ? result.value : 0));
  return {
    total: values[0] ?? 0,
    emAnalise: values[1] ?? 0,
    concluidas: values[2] ?? 0,
    canceladas: values[3] ?? 0,
    aguardandoPeca: values[4] ?? 0,
    paraConserto: values[5] ?? 0,
    hojeAbertas: values[6] ?? 0,
    hojeFechadas: values[7] ?? 0,
    unavailable,
  };
}
