import { useListPedidos } from "@workspace/api-client-react"
        ...
        <CardContent className="p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="space-y-1.5 lg:col-span-2">
            ...
            <div className="relative">
              ...
              <Input 
                ...
                onChange={(e) => { setFilter("cliente", e.target.value); setPage(1); }}
              />
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <Input 
              ...
              onChange={(e) => { setFilter("mesAno", e.target.value); setPage(1); }}
            />
          </div>
        </CardContent>
      </Card>
          ...
          <Table>
            <TableHeader className="sticky top-0 bg-background z-10 shadow-sm">
              <TableRow>
                ...
                <TableHead className="w-[120px] text-right">Valor Venda</TableHead>
                <TableHead className="w-[150px]">Representante</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {isLoading ? (
                ...
                <TableRow>
                  <TableCell colSpan={7}>
                    <EmptyState 
                      title="Nenhum pedido" 
                      description="A base de pedidos está vazia ou não corresponde aos filtros."
                    />
                  </TableCell>
                </TableRow>
              ) : (
                data?.data.map((item, idx) => (
                  <TableRow key={`${item.pedido}-${idx}`} className="group">
                    ...
                    <TableCell className="font-medium text-sm">
                      <div className="line-clamp-2" title={item.nome_cliente || ""}>{item.nome_cliente || "-"}</div>
                    </TableCell>
                    <TableCell className="text-sm">
                      <div className="line-clamp-1" title={item.descricao || ""}>{item.descricao || item.produto || "-"}</div>
                    </TableCell>
                    ...
                    <TableCell className="text-xs text-muted-foreground truncate" title={item.nome_repres || ""}>
                      {item.nome_repres || "-"}
                    </TableCell>
                  </TableRow>
                ))
